...
\item \chadded[id=CAP8-Common-Seal]{The Association does not have a common seal.} \chcomment{As per \href{https://www.commerce.wa.gov.au/publications/whats-rules}{Commerce WA} - "If the Association does not want to use a common seal it should include a simple statement in its rules that it has no common seal." }
\end{enumerate}
...
\item A notice or other document that is to be given to a member under these rules is taken not to have been given to the member unless it is in writing and ---
  ...
  \begin{enumerate}
  \item sent by electronic transmission to an appropriate recorded electronic address of the member; or \chcomment{Differs from model rules in that "facsimile" is eliminated as an option, and items a), b) and c) are in a different order. OK.}
